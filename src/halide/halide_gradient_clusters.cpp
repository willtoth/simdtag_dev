...
extern "C" HALIDE_EXPORT_SYMBOL int32_t __TEST123(int32_t x, int32_t y) {
    static int32_t i = 1;
    return i++;
}
HalideExtern_2(int32_t, __TEST123, int32_t, int32_t);
...
class GradientClusters : public Halide::Generator<GradientClusters> {
   public:
    ...
    void generate() {
        ...
        r.where(clamped_input(r.x, r.y, 0) != 127);
        // output.dim(0).set_bounds(0, input.width() * input.height() * 4);

        // gradient_clusters(x, y, c) = cast<uint64_t>(0);
        output(x) = cast<uint64_t>(0);

#define DO_CONN(dx, dy, EXPR_NUM)                                                             \
    Expr value_##EXPR_NUM;                                                                    \
    {                                                                                         \
        Expr rep0 = cast<uint64_t>(clamped_labels(r.x, r.y, 0));                              \
        Expr rep1 = cast<uint64_t>(clamped_labels(r.x + dx, r.y + dy, 0));                    \
        Expr v0 = cast<uint64_t>(clamped_input(r.x, r.y, 0));                                 \
        Expr v1 = cast<uint64_t>(clamped_input(r.x + dx, r.y + dy, 0));                       \
                                                                                              \
        ...
        Expr value_expr =                                                                     \
                hash_value | cast<uint64_t>(r.x) << 13 | cast<uint64_t>(r.y) << 2 | EXPR_NUM; \
