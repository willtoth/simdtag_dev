        ...
        for (int row = 1; row < height; row++) {
            ...
            for (;; runs++) {
                ...
                if (end_pos <= runs_up->end_pos) {
                    if (is_connected(bits_f, cross_st, end_pos)) runs->label = label_solver_.GetLabel(runs_up->label);
                    else runs->label = label_solver_.NewLabel();
                    runs->start_pos = start_pos;
                    runs->end_pos = end_pos;
                    continue;
                }
                ...
                unsigned label;
                if (is_connected(bits_f, cross_st, runs_up->end_pos)) label = label_solver_.GetLabel(runs_up->label);
                else label = 0;
                runs_up++;
